...
import { faPen, faCartPlus } from '@fortawesome/free-solid-svg-icons';
...
const CartProduct: React.FC<CartProductProps> = ({ productName, productCalories, productCarbohydrates, productFibers, productProteins, productCategory, onEdit }) => {
    return (
        <div className="max-w-sm p-6 shadow-inner border-gray-800 rounded-lg dark:bg-gray-800 dark:border-gray-700 w-[290px] h-[350px] bg-gradient-to-bl from-lime-200 to-purple-400">
            ...
            <h5 className="flex place-content-between mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white"><p>{productName}</p>
                <div className="flex items-start">
                    <button onClick={onEdit} data-modal-target="popup-modal" data-modal-toggle="popup-modal" type="button" className="mr-5 text-lime-500 hover:opacity-70"><FontAwesomeIcon icon={faCartPlus} /></button>
                </div>
            </h5>
...
export default function Page() {
    ...
    useEffect(() => {
        ...
        // if (isLoginModalOpen === false && userData && selectedProduct && actionAfterLogin === "product") {
        //     setIsEditModalOpen(true);
        // }
    }, [params]);
    // }, [isLoginModalOpen, userData, selectedProduct, actionAfterLogin]);
            ...
            <div className="flex gap-4 flex-col" style={{ maxWidth: 'calc(100vw - 18em)' }}>
                <div className="p-5 sm:rounded-lg gap-4 flex flex-wrap flex-col md:flex-row max-h-[calc(100vh-15em)] overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar]:hover:cursor-pointer [&::-webkit-scrollbar-track]:bg-gray-100   [&::-webkit-scrollbar-thumb]:bg-gray-300   dark:[&::-webkit-scrollbar-track]:bg-neutral-700   dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500 pr-[5px]">
                    {productList && productList.map((product) => {
                        return (
                            <CartProduct onEdit={() => handleOpenEditModal(product)} productId={product.id} productCategory={product.category} productName={product.name} productCalories={product.calories} productProteins={product.proteins} productFibers={product.fibers} productCarbohydrates={product.carbohydrates} key={product.id} />
                        )
                    })}
                </div>
                <div>
                ...
                {isEditModalOpen && selectedProduct && (
                    ...
                    >
                        ...
                        <InputField
                            ...
                            //     setSelectedProduct({ ...selectedProduct, quantity_in_grams: numericValue });
                            // }}
